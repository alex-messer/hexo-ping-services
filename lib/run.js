...
function resolveStateFilePath(stateFile, baseDir) {
  ...
  if (stateAbs !== baseAbs && !stateAbs.startsWith(baseAbs + path.sep)) {
    const err = new Error(
      `hexo-ping-services: ping.state_file (${stateFile}) must be inside hexo.base_dir`
    );
    err.absolutePaths = { stateAbs, baseAbs };
    throw err;
  }
  return stateAbs;
}

async function assertComposedUrl(label, value, options) {
  try {
    await assertPublicHttpUrl(value, options);
  } catch (err) {
    const e = new Error(`hexo-ping-services: composed ${label} is not a public http(s) URL: ${err.message}`);
    e.cause = err;
    throw e;
  }
}
...
async function runPing(hexo, options = {}) {
  ...
  const guardOpts = { resolveDns: config.validateDns };
  if (config.indexnow.enabled) {
    await assertComposedUrl('indexnow keyLocation', keyLocation, guardOpts);
  }
  if (feedUrl && config.xmlrpc.enabled && config.xmlrpc.endpoints.length) {
    await assertComposedUrl('xmlrpc feedUrl', feedUrl, guardOpts);
  }
  if (websubFeedUrl && config.websub.enabled && config.websub.hubs.length) {
    await assertComposedUrl('websub feedUrl', websubFeedUrl, guardOpts);
  }
  if (config.indexnow.enabled || (config.xmlrpc.enabled && config.xmlrpc.endpoints.length)) {
    await assertComposedUrl('site url', siteUrl, guardOpts);
  }
  ...
  const [indexnowResults, xmlrpcResults, websubResults] = await Promise.all([
    ...
    (config.xmlrpc.enabled && config.xmlrpc.endpoints.length)
      ? pingAll(
          ...
          siteUrl,
          feedUrl,
          { timeoutMs: config.timeoutMs, concurrency: config.concurrency, validateDns: config.validateDns }
        )
      : [],
  ...
  if (anySuccess) {
    ...
    await commit(stateFilePath, state, pingedItems);
  }
